...
def get_pages(url):
    headers={
        'Cookie': 'LIVE_BUVID=AUTO6915375856445563; stardustvideo=1; buvid3=EEB30FF5-70B4-4E16-B15A-94FE3F7AF07F163007infoc; rpdid=kmpsximoqmdoskmsqsxqw; _uuid=0E9564E0-EFCD-80A6-819D-B0BF6986056427195infoc; finger=edc6ecda; CURRENT_FNVAL=16; sid=4ih3nm1u; fts=1540866975; UM_distinctid=166c2db295180-082aa06c2afa6d-47e1039-100200-166c2db2952110',
        'Host': 'www.bilibili.com',
        'Referer': 'https: // space.bilibili.com / 27580511 / ',
    ...
    pages=soup.find('div',class_='bb-comment')
    print(pages)
def get_comment(url):
    headers={
        'Cookie': 'LIVE_BUVID=AUTO6915375856445563; stardustvideo=1; buvid3=EEB30FF5-70B4-4E16-B15A-94FE3F7AF07F163007infoc; rpdid=kmpsximoqmdoskmsqsxqw; finger=edc6ecda; CURRENT_FNVAL=16; sid=4ih3nm1u; fts=1540866975; UM_distinctid=166c2db295180-082aa06c2afa6d-47e1039-100200-166c2db2952110',
...
if __name__=='__main__':
    ...
    #print(video)
    page_url='https://www.bilibili.com/video/av12551207'
    #pages(page_url)
    pages=183
    url = 'https://api.bilibili.com/x/v2/reply?callback=jQuery17207334597019735183_1540982500519&jsonp=jsonp&pn=1&type=1&oid=12551207&sort=0&_=' + str(
        int(time.time() * 1000))
    headers = {
        'Cookie': 'LIVE_BUVID=AUTO6915375856445563; stardustvideo=1; buvid3=EEB30FF5-70B4-4E16-B15A-94FE3F7AF07F163007infoc; rpdid=kmpsximoqmdoskmsqsxqw; finger=edc6ecda; CURRENT_FNVAL=16; sid=4ih3nm1u; fts=1540866975; UM_distinctid=166c2db295180-082aa06c2afa6d-47e1039-100200-166c2db2952110',
        'Host': 'api.bilibili.com',
    ...
    for rep in reps:
        ...
        df = df.append(inf, ignore_index=True)
        #print(inf)
    df.to_csv('runningman2017.csv', index=False, mode='a', header=False,encoding='utf-8',columns=['评论','time','floor','点赞','性别','name','等级','回复'])
